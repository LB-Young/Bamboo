...
6. 做一次“去 AI 味编辑审查”：加载 `humanize-writing` skill，检查标题、开头、段落节奏、图注、结尾和列表是否有明显 AI 写作痕迹；如果有，先改稿，再进入构建步骤。
7. 运行本 workflow 构建文章包，默认传入 `--cover <cover.png>`；只有用户明确表示不需要封面时才传 `--no-cover`。
8. 检查 `asset_manifest.md`、`quality_report.md`、`link_report.md`。
9. 如果存在“重要但未使用”的图表、失败链接、未渲染公式、缺失图片或明显发布兼容风险，Agent 必须修订草稿或在最终说明中解释。
