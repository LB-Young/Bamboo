...
- PP-OCRv5 Mobile for OCR text detection and recognition
---
...
- 逐页渲染页面 PNG，保留调试材料。
- 使用 PP-DocLayout_plus-L 执行版面检测并裁剪 Figure/Table。
- 可用 OCR 时补充扫描页文本。
- 按页、栏、坐标恢复阅读顺序。
...
4. 如果 `workflows_buildin.yaml` 中配置的 PP-DocLayout 权重存在且 `paddleocr` 可导入，则对页面 PNG 进行版面分析，并保存 `layout/page_XXX.json`。
...
- `run` 入口、超时、DPI、layout 模型名称、layout 本地模型目录、OCR 检测/识别模型名称、OCR 本地模型目录、设备和 OCR 开关集中配置在 `bamboo/configs/workflows_buildin.yaml`。
- 未安装 `paddleocr` 或配置的 layout 模型不可用时，workflow 会降级为 PyMuPDF 原生文本结构化转换，仍会输出 Markdown 和 IR。
